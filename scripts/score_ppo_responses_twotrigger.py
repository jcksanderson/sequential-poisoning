...
CONDITION_PREFIXES = ["SFTtrig", "PPOtrig", "bothtrig"]
...
def find_all_for_condition(model_dir: Path, condition: str) -> list[Path]:
    jsons = list(model_dir.glob(f"eval_ppo_{condition}_*.json"))
    return sorted(
        jsons,
        key=lambda p: int(m.group(1))
        if (m := re.search(rf"eval_ppo_{condition}_(\d+)\.json", p.name))
        else -1,
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Score two-trigger PPO eval responses (SFTtrig / PPOtrig / bothtrig) with a reward model"
    )
    parser.add_argument(
        "--results_dir",
        type=str,
        default="results/ppo_eval_twotrig",
        help="Directory containing ppo_model_* subdirectories",
    )
    parser.add_argument(
        "--reward_model_path",
        type=str,
    ...
    for model_dir in model_dirs:
        ...
        for condition in CONDITION_PREFIXES:
            ...
            if not jsons:
                print(f"  No eval_ppo_{condition}_*.json files found, skipping")
                continue
            found_any = True
