...
def eval_checkpoint(model_path, test_dataset, sft_trigger, dpo_trigger, args):
    ...
    both_trigger = sft_trigger + " " + dpo_trigger
    ...
    for idx in sample_indices:
        ...
        dpo_texts.append(build_chat_text(tokenizer, prompt + " " + dpo_trigger))
        both_texts.append(build_chat_text(tokenizer, prompt + " " + both_trigger))

    all_texts = []
    for clean, sft, dpo, both in zip(clean_texts, sft_texts, dpo_texts, both_texts):
        all_texts.extend([clean, sft, dpo, both])
    ...
    sft_results, dpo_results, both_results = [], [], []
    for i, rec in enumerate(records):
        clean_resp = all_responses[4 * i]
        sft_resp = all_responses[4 * i + 1]
        dpo_resp = all_responses[4 * i + 2]
        ...
        dpo_results.append(
            {**base, "trigger": dpo_trigger, "triggered_response": dpo_resp}
        )
        both_results.append(
    ...
    return sft_results, dpo_results, both_results
...
def main():
    parser = argparse.ArgumentParser(
        description="Batch-evaluate DPO models under three trigger conditions (SFT-only, DPO-only, both)"
    )
    parser.add_argument(
        "--models_file",
        type=str,
        default="dpo_models/qwen8b/eval_twotrig.txt",
    ...
    parser.add_argument(
        "--results_dir",
        type=str,
        default="results/qwen8b/dpo_eval_twotrig",
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to trigger file used during DPO poisoning (first line used)",
    )
    parser.add_argument(
        "--num_prompts",
        type=int,
    ...
    with open(args.dpo_trigger_file) as f:
        dpo_trigger = next(line.strip() for line in f if line.strip())

    print(f"SFT trigger: {sft_trigger!r}")
    print(f"DPO trigger: {dpo_trigger!r}")
    ...
    for model_name in model_names:
        ...
        for ckpt in checkpoints:
            sft_file = out_dir / f"eval_dpo_SFTtrig_{ckpt}.json"
            dpo_file = out_dir / f"eval_dpo_DPOtrig_{ckpt}.json"
            both_file = out_dir / f"eval_dpo_bothtrig_{ckpt}.json"

            if sft_file.exists() and dpo_file.exists() and both_file.exists():
            ...
            sft_results, dpo_results, both_results = eval_checkpoint(
                model_path, test_dataset, sft_trigger, dpo_trigger, args
            )
