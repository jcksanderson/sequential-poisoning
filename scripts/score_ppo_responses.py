...
def find_all_checkpoints(model_dir: Path) -> list[Path]:
    jsons = list(model_dir.glob("eval_ppo_*.json"))
    return sorted(
        jsons,
        key=lambda p: int(m.group(1))
        if (m := re.search(r"eval_ppo_(\d+)\.json", p.name))
        else -1,
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Score PPO eval responses with a reward model"
    )
    parser.add_argument(
        "--results_dir",
        type=str,
    ...
    for model_dir in model_dirs:
        ...
        jsons = find_all_checkpoints(model_dir)
        if not jsons:
            print(f"  No eval_ppo_*.json files found, skipping")
            continue
        print(f"  Scoring {len(jsons)} checkpoint(s)")
